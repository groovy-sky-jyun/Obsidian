...
위는 <span style="color:rgb(205, 174, 234)">*현재 캐릭터의 캡슐 컴포넌트 하위에 카메라를 부착*</span>하는 코드이다.
1. #GetCapsuleComponent 를 이용하여 <span style="color:rgb(205, 174, 234)">*캐릭터의 캡슐 컴포넌트(UCapsuleComponent)를 가져온다.*</span> #ACharacter 클래스에 있는 함수이며 #UCapsuleComponent 는 Collision을 가지고 있다. 
<br>
...
2. 반환된 #UCapsuleComponent 를 #USceneComponent 로 <span style="color:rgb(205, 174, 234)">*캐스팅*</span>(타입변환)한다. 
	...
	>
	>그렇기 때문에 붙이고자 하는 컴포넌트(UCapsuleComponent)의 부모(최상위) 컴포넌트에 FPSCameraComponent를 붙여주어야 문제가 없다. (그렇지 않으면 자식과 부모가 다르게 동작하여 문제가 발생할 수 있다.)
	>
	>그래서 #SetupAttachment 함수를 사용할 때에는 대부분 #USceneComponent 로의 캐스팅이 필요하다.
